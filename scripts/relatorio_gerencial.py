...
def processar_dados_gerenciais_df(df: pd.DataFrame, periodo_inicio: date | None, periodo_fim: date | None):
    ...
    df["Meta_Linha"] = _to_num(df["Meta_Linha"]) if "Meta_Linha" in df.columns else 0.0
    df["Litros_Esperados"] = _to_num(df["Litros_Esperados"]) if "Litros_Esperados" in df.columns else 0.0
    ...
    print(f"🔎 [Filtros] Após recálculo KML: {len(df)}")

    df_clean = df[(df["kml"] >= 1.5) & (df["kml"] <= 5)].copy()
    print(f"🔎 [Filtros] Após faixa KML 1.5-5.0: {len(df_clean)}")

    if df_clean.empty:
        print("❌ DataFrame zerado após filtros.")
        print("📌 Colunas disponíveis:", list(df.columns))
        try:
            print("📌 Amostra após normalização:", df.head(5).to_dict(orient="records"))
        except Exception:
            pass
